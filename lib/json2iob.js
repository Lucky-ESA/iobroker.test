...
module.exports = class Json2iob {
    ...
    async parse(elements, path, element, options) {
        try {
            ...
            const lastsplit = path.split('.')[path.split('.').length-1]
            if (elements[lastsplit]) {
                options["channelName"] = elements[lastsplit]["name"];
            } else if (options["channelName"] !== null) {
            ...
            if (Array.isArray(element)) {
                this.extractArray(elements, element, "", path, options);
                return;
            }
            objectKeys.forEach(async (key) => {
                ...
                if (Array.isArray(element[key])) {
                    this.extractArray(elements, element, key, path, options);
                } else if (element[key] !== null && typeof element[key] === "object") {
                    this.parse(elements, path + "." + key, element[key], options);
                } else {
                    if (!this.alreadyCreatedOBjects[path + "." + key]) {
                        if (key === "state" || key === "dimlevel") {
                            options["write"] = true;
                        } else {
                            options["write"] = false;
                        }
    ...
    extractArray(elements, element, key, path, options) {
        try {
            if (key) {
                element = element[key];
            }
            element.forEach(async (arrayElement, index) => {
                ...
                if (typeof arrayElement === "string") {
                    this.parse(elements, path + "." + key + "." + arrayElement, arrayElement, options);
                    return;
                }
                ...
                if (
                    ...
                    if (!this.alreadyCreatedOBjects[path + "." + subKey]) {
                        if (subKey === "state" || subKey === "dimlevel") {
                            options["write"] = true;
                        } else {
                            options["write"] = false;
                        }
                ...
                this.parse(elements, path + "." + arrayPath, arrayElement, options);
            });
        } catch (error) {
